...
Empirical Wavelet Transform + RVFL ensemble.
Decomposes the signal using EWT, trains RVFL on each component,
and sums predictions.

If ewtpy is not installed, falls back to a standard RVFL.
"""
...
def decompose_ewt(signal, N=5):
    """
    Decompose signal using Empirical Wavelet Transform.
    ...
    Returns
    -------
    ewt_components : np.ndarray, shape (N, len(signal))
    """
    if not HAS_EWT:
        return None
    try:
        ewt, _, _ = ewtpy.EWT1D(signal, N=N)
        return ewt.T  # shape: (N, len(signal))
    except Exception:
        return None
...
class EWTRVFL:
    """EWT-RVFL ensemble model."""

    def __init__(self, window_size, hidden_dim=100, ridge_alpha=0.1,
                 input_scaling=1.0, N=5, seed=42):
    ...
    def fit(self, train_series, y_train_unused=None):
        ...
        if not self.use_ewt:
            # Fallback: single RVFL on raw series
    ...
    def predict(self, test_series):
        """
        Predict using EWT-RVFL ensemble.
